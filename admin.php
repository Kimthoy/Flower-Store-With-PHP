...
$select_products = mysqli_query($conn, "SELECT * FROM `products` LIMIT $start, $products_per_page");

// Query to count total products in the database
...
<section class="display-product-table">
   ...
   <div class="pagination">
      ...
      // Previous Page Link
      if ($page > 1) {
         echo '<a href="admin.php?page=' . ($page - 1) . '" class="prev" style="font-size:20px;"> ← Previous</a>';
      }

      // Page Numbers
      for ($i = 1; $i <= $total_pages; $i++) {
         echo '<a href="admin.php?page=' . $i . '" class="page-number ' . ($i == $page ? 'active' : '') . '">' . $i . '</a>';
      }

      // Next Page Link
      if ($page < $total_pages) {
         echo '<a href="admin.php?page=' . ($page + 1) . '" class="next" style="font-size:20px;"> Next → </a>';
      }
      ?>
   </div>
